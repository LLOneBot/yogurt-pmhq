...
internal typealias SWord = Long
internal typealias HalfWord = UInt

// Constants
internal const val WORD_BITS: Int = 64
internal const val WORD_BYTES: Int = 8
internal val WORD_MAX: Word = ULong.MAX_VALUE
internal val WORD_MSB: Word = 1UL shl (WORD_BITS - 1)
internal const val CACHE_LINE_SIZE: Int = 64

// Memory pool configuration for temporary allocations
internal const val POOL_BLOCK_SIZE: Int = 64  // Words per block
internal const val POOL_MAX_BLOCKS: Int = 128

// Karatsuba threshold (in words)
internal const val KARATSUBA_THRESHOLD: Int = 32

// Montgomery threshold (in words)
internal const val MONTGOMERY_THRESHOLD: Int = 8
...
internal fun addWithCarry(a: Word, b: Word, carryIn: Boolean): Pair<Word, Boolean> {
...
internal fun subWithBorrow(a: Word, b: Word, borrowIn: Boolean): Pair<Word, Boolean> {
...
internal fun mulWord(a: Word, b: Word): Pair<Word, Word> {
...
// Count leading zeros
internal fun countLeadingZeros(x: Word): Int {
    if (x == 0UL) return WORD_BITS
    return x.countLeadingZeroBits()
}

// Count trailing zeros
internal fun countTrailingZeros(x: Word): Int {
    if (x == 0UL) return WORD_BITS
    return x.countTrailingZeroBits()
}

// Constant-time comparison utilities
internal fun constantTimeEq(a: Word, b: Word): Word {
    return (a xor b).inv() + 1UL  // Returns WORD_MAX if equal, 0 otherwise
}

internal fun constantTimeSelect(mask: Word, a: Word, b: Word): Word {
